...
// ---------------------------------------------------------------------------------
MechWalkTest::MechWalkTest(void): rel_Speed(0.0f),
...
bool MechWalkTest::frameRenderingQueued(const Ogre::FrameEvent &evt) {
	...
	if (rel_Speed > 0) {
		...
		_scn_Mech->translate(
			Ogre::Vector3(Ogre::Math::Cos(rel_MechDirection), 0, Ogre::Math::Sin(rel_MechDirection)) * rel_Speed * evt.
			timeSinceLastFrame * 2.5);
	...
	if (!processUnbufferedInput(evt)) { return false; } else if (!frameStarted(evt)) { return false; }
	return ret;
}
...
bool MechWalkTest::processUnbufferedInput(const Ogre::FrameEvent &evt) {
	...
	if (mKeyboard->isKeyDown(OIS::KC_Z)) {
		...
		mCamera->setPosition(Ogre::Vector3(rel_OrbitRadius * Ogre::Math::Cos(rel_CameraAngle), 200,
		                                   rel_OrbitRadius * Ogre::Math::Sin(rel_CameraAngle)));
